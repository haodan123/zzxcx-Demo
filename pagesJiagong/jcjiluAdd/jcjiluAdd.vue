	...
	<view class="viewport">
		...
		<view class="formCard">
			<uni-forms :modelValue="formData" ref="form" label-width='100px' label-align='left'>
				...
				<uni-forms-item :rules="[{required: true,errorMessage: '请选择加工厂名称'}]" label="加工厂名称:" name="jgc" required
					style="position: relative;">
					<uani-datapicker v-model="formData.jgc" :localdata="jgcList" placeholder="请选择" @onSelect="ssnsSelect2">
					</uani-datapicker>
				</uni-forms-item>

				<uni-forms-item :rules="[{required: true,errorMessage: '请选择检测批次'}]" label="检测批次:" name="jcpc" required
					style="position: relative;">
				...
				<uni-forms-item :rules="[{required: true,errorMessage: '请选择检测类别'}]" label="检测类别:" name="jclb" required
					style="position: relative;">
					<uani-datapicker v-model="formData.jclb" :localdata="jclbList" placeholder="请选择" @onSelect="ssnsSelect4">
					</uani-datapicker>
				</uni-forms-item>
				<uni-forms-item :rules="[{required: true,errorMessage: '请选择检测结果'}]" label="检测结果:" name="jcjg" required
					style="position: relative;">
					<uani-datapicker v-model="formData.jcjg" :localdata="jcjgList" placeholder="请选择" @onSelect="ssnsSelect5">
					</uani-datapicker>
				</uni-forms-item>


			<uni-forms-item label="生产日期:" name='sctime' :rules="[{required: true,errorMessage: '请选择生产日期'}]" required>
				<uni-datetime-picker type="datetime" returnType="date" @change="changTime" :clear-icon="false"
					v-model="formData.sctime" />
			</uni-forms-item>
			<uni-forms-item label="送检日期:" name='sjtime' :rules="[{required: true,errorMessage: '请选择送检日期'}]" required>
				<uni-datetime-picker type="datetime" returnType="date" @change="changTime" :clear-icon="false"
					v-model="formData.sjtime" />
			</uni-forms-item>
			<uni-forms-item label="检测日期:" name='jctime' :rules="[{required: true,errorMessage: '请选择检测日期'}]" required>
				<uni-datetime-picker type="datetime" returnType="date" @change="changTime" :clear-icon="false"
					v-model="formData.jctime" />
			</uni-forms-item>


				<uni-forms-item :rules="[{required: true,errorMessage: '请上传检测报告'}]" label="检测报告:" name="picture" required>
				</uni-forms-item>
...
<script setup>
	...
	const jgcList = ref([]) //加工厂数组
	const jcpcList = ref([]) //检测批次数组
	const jclbList = ref([]) //检测类别数组
	const jcjgList = ref([]) //检测结果数组
