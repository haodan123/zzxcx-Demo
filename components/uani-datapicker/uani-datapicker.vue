<template>
	<view>
		<view @click.stop.prevent="openpop()" class="select-box">
			...
			<uni-icons v-if="clear&&inputtext" type="clear" color="#c0c4cc" class="icon1" size="22"
				@click.stop.prevent="clearValue"></uni-icons>
		...
		<view class="uni-data-tree-dialog" :class="{'uni-data-tree-dialog-show':isOpened}" v-if="isOpened">
			...
			<uni-transition :duration="200" :mode-class="['slide-bottom']" :show='showTrans'>
				<view class="popupbox">
					...
					<scroll-view scroll-y="true" class="popfor" v-if="poplist.length>0">
