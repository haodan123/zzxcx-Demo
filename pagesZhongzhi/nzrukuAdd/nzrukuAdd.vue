	...
	<view class="viewport">
		...
		<view class="formCard">
			<uni-forms :modelValue="formData" ref="form" label-width='80px' label-align='left'>
				...
				<uni-forms-item :rules="[{required: true,errorMessage: '请选择类型'}]" label="类型:" name="leixin" required
					style="position: relative;">
					<uani-datapicker v-model="formData.leixin" :localdata="leixinList" placeholder="请选择" @onSelect="ssnsSelect">
					</uani-datapicker>
				</uni-forms-item>
				<uni-forms-item v-if="formData.leixin==1" :rules="[{required: true,errorMessage: '请选择退用编号'}]" label="退用编号:" name="spfl" required
					style="position: relative;">
					<uni-easyinput type="text" v-model="formData.zmsl"  placeholder="请输入退用编号" />
